...
class DatabaseManager:
    ...
    def get_token_for_sharpi_melk(self):
        ...
        with sqlite3.connect(self.db_filename) as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT token,number FROM personal_number WHERE added != 7 order by id ASC limit 1')
            items = cursor.fetchall()
        return [item for item in items]
    ...
    def update_token_for_sharpi_melk(self, token):
        ...
        with sqlite3.connect(self.db_filename) as conn:
            cursor = conn.cursor()
            try:
                cursor.execute('UPDATE personal_number SET added = 7 WHERE token = ?', token)
            except sqlite3.IntegrityError:
                # Handle the case where the token already exists (do nothing or log if necessary)
                print(f"Token token already exists in the database.")
            conn.commit()
